...
cmd({
  ...
  try {
    ...
    await conn.sendMessage(from, {
      ...
      caption: `📥 *KASHMIRI-MD FB DOWNLOADER..🚀*\n\n*QUAILTY•${videoData.quality}*\n\n🔗 *ᴀᴡᴀɪs ᴍᴅ*`
    }, { quoted: m });
...
async (conn, mek, m, { from, args, q, reply, react }) => {
    try {
        ...
        const captionText = `*KASHMIRI-MD IG DOWNLOADER🚀*` +
                            `📸 *Instagram Post* 📸\n\n` +
                            `👤 *User:* ${fullname} (@${username})\n` +
                            `❤️ *Likes:* ${likes}\n💬 *Comments:* ${comments}\n👥 *Followers:* ${followed}\n` +
                            `📝 *Caption:*\n${caption || "THE KASHMIRI-MD API."}`;
...
async (conn, mek, m, { from, args, q, reply }) => {
    try {
        ...
        reply("*_KASHMIRI-MD DOWNLOADING TIKTOK VIDEO , PLEASE WAIT...🚀_*");
        ...
        const caption = `🎵 *KASHMIRI-MD TIKTOK VIDEO* 🎵\n\n` +
